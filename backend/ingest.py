...
# def extract_text_from_pdf(pdf_path):
#     """Uses pdfplumber to extract text (better than PyPDF)."""
#     text = ""
#     with pdfplumber.open(pdf_path) as pdf:
#         for page in pdf.pages:
#             page_text = page.extract_text()
...
#         elif file_path.lower().endswith('.pdf'):
#             # NEW LOGIC: Use pdfplumber
#             raw_text = extract_text_from_pdf(file_path)
#             if not raw_text.strip():
#                 return {"error": "PDF is scanned or empty. Please convert to images."}
#             pages = [Document(page_content=raw_text, metadata={"source": file_path, "page": 1})]
...
#     if not chunks:
#         return {"error": "No text extracted (chunks are empty)."}
